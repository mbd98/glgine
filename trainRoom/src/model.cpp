...
// https://learnopengl.com/Model-Loading/Model
    static std::vector<Texture> loadMaterialTextures(const std::string &modelName, aiMaterial *mat, aiTextureType type,
        ...
        for (uint i = 0; i < mat->GetTextureCount(type); i++) {
            ...
            if (loaded_textures.count(std::string(str.C_Str())) > 0) {
                ...
                Texture tex;
                tex.texId = loadTexture(
                        ("../trainRoom/assets/models/" + modelName + "/" + std::string(str.C_Str())).c_str());
                tex.type = typeName;
    ...
    static Mesh *processMesh(const std::string &modelName, aiMesh *mesh, const aiScene *scene) {
        ...
        for (uint i = 0; i < mesh->mNumVertices; i++) {
            vertices.emplace_back(convertVector(mesh->mVertices[i]),
                                  ...
                                  mesh->mTextureCoords[0] ? convertVector(mesh->mTextureCoords[0][i]) : glm::vec2(
                                          0.0f));
        }
        ...
        if (mesh->mMaterialIndex >= 0) {
            ...
            std::vector<Texture> diffuse = loadMaterialTextures(modelName, material, aiTextureType_DIFFUSE,
                                                                "texture_diffuse");
            textures.insert(textures.end(), diffuse.begin(), diffuse.end());
            std::vector<Texture> specular = loadMaterialTextures(modelName, material, aiTextureType_SPECULAR,
                                                                 "texture_specular");
...
// https://learnopengl.com/Model-Loading/Mesh
    ...
    Model::Model(const std::string &name) {
        ...
        Assimp::Importer in;
        const aiScene *scene = in.ReadFile("../trainRoom/assets/models/" + name + "/" + name + ".obj",
